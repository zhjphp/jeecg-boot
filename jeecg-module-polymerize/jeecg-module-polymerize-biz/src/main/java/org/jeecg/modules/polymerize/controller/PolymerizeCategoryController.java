...
public class PolymerizeCategoryController {
    ...
    public Result<IPage<PolymerizeCategory>> queryPageList(PolymerizeCategory polymerizeCategory,
        ...
        if(oConvertUtils.isEmpty(polymerizeCategory.getPid())){
            polymerizeCategory.setPid("0");
        }
        Result<IPage<PolymerizeCategory>> result = new Result<IPage<PolymerizeCategory>>();
        //------------------------------------------------------------------------------------------------
        //是否开启系统管理模块的多租户数据隔离【SAAS多租户模式】
        if(MybatisPlusSaasConfig.OPEN_SYSTEM_TENANT_CONTROL){
            polymerizeCategory.setTenantId(oConvertUtils.getInt(TenantContext.getTenant(),0));
        }
        //------------------------------------------------------------------------------------------------
        ...
        String name = polymerizeCategory.getName();
        String code = polymerizeCategory.getCode();
        //QueryWrapper<PolymerizeCategory> queryWrapper = new QueryWrapper<PolymerizeCategory>();
        if(StringUtils.isBlank(name)&&StringUtils.isBlank(code)){
            queryWrapper.eq("pid", polymerizeCategory.getPid());
        }
    ...
    public Result<List<PolymerizeCategory>> queryPageList(PolymerizeCategory polymerizeCategory, HttpServletRequest req) {
        //------------------------------------------------------------------------------------------------
        //是否开启系统管理模块的多租户数据隔离【SAAS多租户模式】
        if(MybatisPlusSaasConfig.OPEN_SYSTEM_TENANT_CONTROL){
            polymerizeCategory.setTenantId(oConvertUtils.getInt(TenantContext.getTenant(), 0));
        ...
        //------------------------------------------------------------------------------------------------
        Result<List<PolymerizeCategory>> result = new Result<List<PolymerizeCategory>>();
        QueryWrapper<PolymerizeCategory> queryWrapper = QueryGenerator.initQueryWrapper(polymerizeCategory, req.getParameterMap());
    ...
    public Result<PolymerizeCategory> add(@RequestBody PolymerizeCategory polymerizeCategory) {
        Result<PolymerizeCategory> result = new Result<PolymerizeCategory>();
        try {
    ...
    public Result<PolymerizeCategory> edit(@RequestBody PolymerizeCategory polymerizeCategory) {
        Result<PolymerizeCategory> result = new Result<PolymerizeCategory>();
        PolymerizeCategory polymerizeCategoryEntity = polymerizeCategoryService.getById(polymerizeCategory.getId());
        if(polymerizeCategoryEntity==null) {
    ...
    public Result<PolymerizeCategory> delete(@RequestParam(name="id",required=true) String id) {
        Result<PolymerizeCategory> result = new Result<PolymerizeCategory>();
        PolymerizeCategory polymerizeCategory = polymerizeCategoryService.getById(id);
        if(polymerizeCategory==null) {
            result.error500("未找到对应实体");
        }else {
            this.polymerizeCategoryService.deletePolymerizeCategory(id);
            result.success("删除成功!");
        }
    ...
    public Result<PolymerizeCategory> deleteBatch(@RequestParam(name="ids",required=true) String ids) {
        Result<PolymerizeCategory> result = new Result<PolymerizeCategory>();
        if(ids==null || "".equals(ids.trim())) {
            result.error500("参数不识别！");
        }else {
            this.polymerizeCategoryService.deletePolymerizeCategory(ids);
            result.success("删除成功!");
        }
        return result;
    }
    ...
    public Result<PolymerizeCategory> queryById(@RequestParam(name="id",required=true) String id) {
        ...
        if(polymerizeCategory==null) {
            ...
            result.setResult(polymerizeCategory);
            result.setSuccess(true);
        }
        return result;
    }
    ...
    public ModelAndView exportXls(HttpServletRequest request, PolymerizeCategory polymerizeCategory) {
        ...
        // Step.1 组装查询条件查询数据
        QueryWrapper<PolymerizeCategory> queryWrapper = QueryGenerator.initQueryWrapper(polymerizeCategory, request.getParameterMap());
        List<PolymerizeCategory> pageList = polymerizeCategoryService.list(queryWrapper);
        ...
        if(oConvertUtils.isEmpty(selections)) {
            ...
            List<PolymerizeCategory> exportList = pageList.stream().filter(item -> selectionList.contains(item.getId())).collect(Collectors.toList());
            mv.addObject(NormalExcelConstants.DATA_LIST, exportList);
        }
        //导出文件名称
        mv.addObject(NormalExcelConstants.FILE_NAME, "分类字典列表");
        mv.addObject(NormalExcelConstants.CLASS, PolymerizeCategory.class);
    ...
    public Result<?> importExcel(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
            ...
            try {
                List<PolymerizeCategory> listPolymerizeCategorys = ExcelImportUtil.importExcel(file.getInputStream(), PolymerizeCategory.class, params);
                //按照编码长度排序
                Collections.sort(listPolymerizeCategorys);
                log.info("排序后的list====>",listPolymerizeCategorys);
                for (int i = 0; i < listPolymerizeCategorys.size(); i++) {
                    PolymerizeCategory polymerizeCategoryExcel = listPolymerizeCategorys.get(i);
                    String code = polymerizeCategoryExcel.getCode();
                    if(code.length()>3){
                        String pCode = polymerizeCategoryExcel.getCode().substring(0,code.length()-3);
                        log.info("pCode====>",pCode);
                        String pId=polymerizeCategoryService.queryIdByCode(pCode);
                        log.info("pId====>",pId);
                        if(StringUtils.isNotBlank(pId)){
                            polymerizeCategoryExcel.setPid(pId);
                        }
                    }else{
                        polymerizeCategoryExcel.setPid("0");
                    }
                    try {
                        polymerizeCategoryService.save(polymerizeCategoryExcel);
                        successLines++;
                    } catch (Exception e) {
                        errorLines++;
    ...
    public Result<PolymerizeCategory> loadOne(@RequestParam(name="field") String field,@RequestParam(name="val") String val) {
        Result<PolymerizeCategory> result = new Result<PolymerizeCategory>();
        try {
            //update-begin-author:taoyan date:2022-5-6 for: issues/3663 sql注入问题
            boolean isClassField = SqlInjectionUtil.isClassField(field, PolymerizeCategory.class);
            if (!isClassField) {
                return Result.error("字段无效，请检查!");
            }
            //update-end-author:taoyan date:2022-5-6 for: issues/3663 sql注入问题
            QueryWrapper<PolymerizeCategory> query = new QueryWrapper<PolymerizeCategory>();
            query.eq(field, val);
            List<PolymerizeCategory> ls = this.polymerizeCategoryService.list(query);
    ...
    public Result<List<TreeSelectModel>> loadTreeChildren(@RequestParam(name="pid") String pid) {
        ...
        try {
            List<TreeSelectModel> ls = this.polymerizeCategoryService.queryListByPid(pid);
            result.setResult(ls);
            result.setSuccess(true);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public Result<List<TreeSelectModel>> loadTreeRoot(@RequestParam(name="async") Boolean async,@RequestParam(name="pcode") String pcode) {
        Result<List<TreeSelectModel>> result = new Result<List<TreeSelectModel>>();
        try {
            List<TreeSelectModel> ls = this.polymerizeCategoryService.queryListByCode(pcode);
            if(!async) {
    ...
    private void loadAllCategoryChildren(List<TreeSelectModel> ls) {
        for (TreeSelectModel tsm : ls) {
            List<TreeSelectModel> temp = this.polymerizeCategoryService.queryListByPid(tsm.getKey());
    ...
    public Result<?> checkCode(@RequestParam(name="pid",required = false) String pid,@RequestParam(name="code",required = false) String code) {
        ...
        PolymerizeCategory parent = this.polymerizeCategoryService.getById(pid);
    ...
    public Result<List<TreeSelectModel>> loadDict(@RequestParam(name="pid",required = false) String pid,@RequestParam(name="pcode",required = false) String pcode, @RequestParam(name="condition",required = false) String condition) {
        ...
        if(oConvertUtils.isEmpty(pid)){
            if(oConvertUtils.isEmpty(pcode)){
                ...
                if(IPolymerizeCategoryService.ROOT_PID_VALUE.equals(pcode)){
                    pid = IPolymerizeCategoryService.ROOT_PID_VALUE;
                }else{
                    pid = this.polymerizeCategoryService.queryIdByCode(pcode);
                }
                if(oConvertUtils.isEmpty(pid)){
                    result.setSuccess(false);
        ...
        List<TreeSelectModel> ls = polymerizeCategoryService.queryListByPid(pid,query);
        result.setSuccess(true);
        result.setResult(ls);
        return result;
    }
    ...
    public Result<List<String>> loadDictItem(@RequestParam(name = "ids") String ids, @RequestParam(name = "delNotExist", required = false, defaultValue = "true") boolean delNotExist) {
        ...
        // 查询数据
        List<String> textList = polymerizeCategoryService.loadDictItem(ids, delNotExist);
        result.setSuccess(true);
    ...
    public Result<List<DictModel>> loadAllData(@RequestParam(name="code",required = true) String code) {
        ...
        LambdaQueryWrapper<PolymerizeCategory> query = new LambdaQueryWrapper<PolymerizeCategory>();
        if(oConvertUtils.isNotEmpty(code) && !CATEGORY_ROOT_CODE.equals(code)){
            query.likeRight(PolymerizeCategory::getCode,code);
        }
        List<PolymerizeCategory> list = this.polymerizeCategoryService.list(query);
